...
def workload_zipf(prompts, n_requests, a=1.2):
    ...
    # produce indices according to zipf-like distribution
    N = len(prompts)
    ranks = list(range(1, N + 1))
    # compute weights ~ 1 / rank^a
...
def workload_burst(prompts, burst_size=100, idle=0.5):
    # send a burst from a small subset, then idle
...
def run_benchmarks(base_url, prompts, policies, workloads, concurrency=8, policy_param="policy"):
    ...
    for policy in policies:
        for wname, wprompts in workloads:
            ...
            # if server exposes /stats try to grab evictions/hits
            try:
                s = requests.get(base_url.rstrip("/") + "/stats", params={"policy": policy}, timeout=2.0)
                js = try_json(s)
                if js:
...
def visualize_grid(results, policies, workloads_order):
    ...
    The figure will show: hit ratio, avg latency, latency CDF, latency over time, client memory, server evictions (if available)
    """
...
def build_workloads(prompts, max_requests):
    ...
    # add a short burst workload using first 500 prompts
...
def main():
    ...
    # determine base url from host+port unless overridden
    if args.base_url:
